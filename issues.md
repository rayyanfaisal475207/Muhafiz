...
### [Critical] The application's runtime `DATABASE_URL` connects as the Postgres superuser, which unconditionally bypasses every RLS policy — Phase 2's RLS backstop currently provides zero actual protection — **RESOLVED (Phase 0-7 closeout, 2026-07-28, migrations/015_app_least_privilege_role.sql)**

- **Resolution**: Migration 015 creates `muhafiz_app`, a non-superuser, non-BYPASSRLS role granted SELECT/INSERT/UPDATE/DELETE on every application table (explicit per-table, same convention as migration 009's `muhafiz_mcp_readonly`) plus `CREATE ON SCHEMA public` (required because `src/database/postgres.py::init_postgres()` runs `Base.metadata.create_all()` on every startup using this same connection — a necessary, documented exception to pure DML-only). `.env.example`'s `DATABASE_URL` now points at this role by default, with an explicit warning that existing `.env` files will not auto-update. Live-verified against the real instance (`scripts/verify_app_role.py`): confirmed not-superuser, not-BYPASSRLS, full CRUD works on every table, `DROP TABLE`/`CREATE ROLE` correctly denied, and — the actual point of this fix — **RLS is now genuinely enforced**: with `app.rls_active`/`app.case_id` set, a second case's row is truly invisible under this role (previously this same check only ever passed under a separate verification-only role, never the app's real connection). All 8 existing live-Postgres RLS integration tests (`test_rls_integration.py`, `test_postgres_rls.py`) pass under `muhafiz_app`, and `init_postgres()` itself succeeds under it (confirming the `CREATE ON SCHEMA` grant is sufficient).
