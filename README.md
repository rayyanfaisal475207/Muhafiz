...
- **"All Cases" scope, `supervisor` role or higher, no case selected.** Replaces an earlier "No Case" state that retrieved nothing and had no real purpose. In this scope the case selector's role is inverted from within-case scoping: retrieval draws on every case's evidence plus global reference material (`case_id IS NOT NULL OR is_global`, a new always-present `has_case` boolean metadata field on both the vector store and the full-text index — not "no filter", which would also surface content that legitimately has no case attached but isn't meant to be broadly visible) and cross-case tools (`XGRAPH`/`XAGG`/`XNETWORK`) are directly reachable. Gated by the same `CROSS_CASE_ROLES` check the rest of cross-case access already used (`graph_retriever.py`, reused in `orchestrator.py`'s `_build_retrieval_where()`); an `investigator` with no case selected still gets the original reference-lookup-only scope, unchanged. The chat UI's case selector (`CaseSelector.tsx`, a searchable combobox) and landing-page copy/placeholder both reflect which of the three scopes (case-specific / All Cases / reference-only) the current user is actually in.
...
```bash
for f in migrations/0*.sql; do python scripts/apply_migration.py "$f"; done
```

Until 003 is applied, chat attachments are disabled (with an explicit message) and the admin dashboard shows an "instrumentation not applied" banner. Until 004–008 are applied, the Case model, graph, RBAC, Verifier logging, and RLS respectively won't exist — the app does not currently detect and warn about this the way it does for 003. The chain currently runs to `031_muhafiz_app_age_grants.sql` (29 files); two migrations worth calling out specifically since they change what "secure by default" actually means at runtime:

- **`015_least_privilege_app_role.sql`** — creates the `muhafiz_app` role and repoints `DATABASE_URL` at it instead of the Postgres superuser. This is the migration that makes RLS (008) actually enforce anything: a superuser connection has `BYPASSRLS`, so every `FORCE ROW LEVEL SECURITY` policy is silently inert until the app connects as a non-superuser role. Run this before trusting RLS in any environment that matters.
- **`031_muhafiz_app_age_grants.sql`** — grants `muhafiz_app` the AGE (`ag_catalog`, `evidence_graph`, `evidence_graph_eval`) schema/table access it needs, via `ALTER DEFAULT PRIVILEGES` (AGE creates label tables dynamically at query time, so a one-time per-table `GRANT` isn't enough). Without this, every graph route (`GRAPH`/`GRAPH_HYBRID`/`XGRAPH`/`XAGG`/`XNETWORK`) fails outright once the app is running as `muhafiz_app` — this was found live, not in review, after 015 had already been applied elsewhere. `docker-compose.yml`'s Postgres service also needs `-c shared_preload_libraries=age` (requires a container recreate, not just a restart) — without it, `LOAD 'age'` per-connection is superuser-only and `muhafiz_app` can't load the extension at all.
...
The admin app proxies `/api` to `http://127.0.0.1:8001`. An account needs `role = platform-admin` to reach most of it (`role = supervisor` for the graph-review queue and audit logs).
...
**Current test suite** (re-run 2026-08-29, `python -m pytest`, against a live `muhafiz-postgres`/AGE instance): **2077 collected, 0 failed, 0 errors, 5 skipped** (`tests/test_pdf_loader.py`'s 6 real-Docling tests excluded from this run — see below; counted via `--junit-xml`, not the terminal summary line, which this pytest/environment combination stopped printing at some point, a harmless reporting quirk not investigated further here since per-test pass/fail detail is unaffected) — grown from the 1761-collected count recorded 2026-08-26 as the AGE-grants fix, cross-lingual graph name matching, the "All Cases" retrieval scope, and the XGRAPH/XNETWORK completion-budget fix each landed with their own regression coverage. Both frontends are green too: `frontend/` 51 tests / 11 files, `admin-frontend/` 34 tests / 9 files (`npm test -- --run`, same date). `tests/test_pdf_loader.py`'s 6 real-Docling tests are excluded from the count above — they fail in this environment on a genuine, memory-pressure-dependent error (`OSError: The paging file is too small...`, see RUN.md §9), reproduced flakily in past runs (present on some, absent on others depending on what else this machine had done since its last restart), not a code defect; run them separately on a machine with more headroom. Note for anyone re-running this locally: this repo's `.venv` (if present) may be missing `pytest-asyncio`, which surfaces as dozens of `'asyncio' not found in markers configuration` collection errors — not a code regression, an environment gap; use whichever interpreter actually has `pytest-asyncio` installed (check with `python -m pip show pytest-asyncio` before trusting a failing run).

CI (`.github/workflows/ci.yml`) runs three jobs on every push/PR: `backend` (pytest), `rls-integration` (a dedicated Postgres+AGE service container that specifically exercises RLS enforcement under the `muhafiz_app` role, not the superuser connection most of the `backend` job's tests still run under), and `frontend` (build + `npm test` for both `frontend/` and `admin-frontend/`).
...
- **Correction (2026-08-29): the previous version of this line was itself stale.** Entity resolution (`src/graph/entity_resolution.py::_name_similarity()` / `_consonant_skeleton()`) uses a deliberately coarse Urdu-consonant-to-Roman "skeleton" comparison ("ظفر"/"Zafar" both reduce to `zfr`), chosen over a full transliteration library or an LLM call to avoid a heavier dependency and a new failure mode on the merge-decision path. Query-time seed lookup (`graph_retriever.py::_find_seed_nodes`) now uses the same skeleton — stored per-node as `name_skeleton`, computed at write time and backfilled for existing nodes (`scripts/backfill_name_skeletons.py`) — alongside its existing plain-`CONTAINS` match, so a query naming a person only by their Latin-script name does seed-match a node whose `canonical_name` was stored in Urdu script (this used to be a real, confirmed gap; it isn't one any more). Separately, the router's entity extraction can glue a trailing interrogative/relative particle onto an extracted name ("ذیشان" → "ذیشان کن" from "ذیشان کن کیسز سے منسلک ہے؟") — a corrupted string that neither skeleton matching nor translation fixes, since the extra word is in the *name itself*, not a language mismatch; handled by a deterministic particle-stripping fallback (`_strip_trailing_particle()`) plus a clarified extraction prompt.
...
- **`chunk_fulltext` and `ingestion_run_quality` still lack explicit `muhafiz_app` grants** — the same class of bug migration 031 fixed for the AGE schemas, found while diagnosing a real failed query (`fir-430-26`) but not yet fixed for these two tables. Practical effect: BM25 candidate pooling and ingestion-quality rollups may silently no-op or error under the least-privilege role depending on what default privilege the table happened to inherit at creation time, rather than failing loudly. Known, not yet scheduled.
...
**Also fixed since (2026-08-29):** `muhafiz_app` had zero grants on any AGE schema (`ag_catalog`/`evidence_graph`/`evidence_graph_eval`) — migration 015's least-privilege role switch never touched AGE at all, so every graph route failed once RLS's role fix was actually in effect; fixed by migration 031 plus `shared_preload_libraries=age` in `docker-compose.yml`. Cross-case query results for the same underlying entity varied by script (Urdu vs. Roman spelling of a name) because query-time graph seed lookup didn't use the same skeleton-matching entity resolution already used at write time, plus a separate router bug that could glue a trailing particle onto an extracted name — both fixed (`name_skeleton` seed matching, `_strip_trailing_particle()`). `XGRAPH`/`XNETWORK` answers enumerating many cases could get cut off mid-list and fail the Verifier's grounding check because `cloud_max_tokens=500` was sized for the router's tiny JSON classification, not a long enumeration — fixed with a token budget that scales with the number of items being cited.
