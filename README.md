...
| `HARNESS_CUTOVER_ROUTES`                                        | Comma-separated route names (`RAG`, `XGRAPH`, ...) to hand off to the agent harness instead of the orchestrator. Empty = every route stays on the orchestrator | _(empty)_ — set to `RAG` in this deployment's `.env` |
