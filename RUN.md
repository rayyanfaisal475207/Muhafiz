...
| `LLM_QUERY_FALLBACK_ENABLED` | no | Module 178: let the language model write ONE read-only graph query for a cross-case question that no aggregate answers and no plan composes. Default `false`; see `docs/gold-qa-wave2-results/MODULE178_RESULT.md` §6 before turning it on. Runs on `MCP_DATABASE_URL`'s role (migration 033), rolled back always, capped by `LLM_QUERY_FALLBACK_ROW_CAP` (200) and `LLM_QUERY_FALLBACK_TIMEOUT_S` (15). |
