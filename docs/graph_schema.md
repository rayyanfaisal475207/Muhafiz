...
| `CITES` | `Case`/`Incident` → `Case`/`Incident` | `status` (`pending`\|`confirmed`\|`rejected`), `confidence`, `basis`, `as_of`, `source_doc_id` | **Reserved here, written by M6b** (not yet implemented at time of writing) — a confidence-scored FIR→FIR citation parsed from free text (`docs/decisions/0001-muhafiz-api-migration.md`, round-2 review item 3), following the same pending/confirmed/rejected human-review discipline as `SAME_AS` but kept as a distinct edge type: a case-to-case citation is not an entity-identity claim, and forcing it into `SAME_AS` would corrupt `graph_review.py`'s Person/Vehicle-shaped rendering of that queue. |
