...
| `StructuredRecord` | `record_id`, `record_type` | Typed rows (property-tag, court-listing) from `structured_records/*.csv` — no chunking/embedding, mirrors the architecture's evidence-type split. |
...
| `OWNS` / `REGISTERED_TO` | `Person` → `Vehicle`\|`PhoneNumber` | `as_of`, `source_doc_id` | |
| `LOCATED_AT` | any entity → `Address` | `as_of`, `source_doc_id` | |
| `INVOLVED_IN` | `Person` → `Incident` | `role`, `confidence`, `as_of` | |
| `PART_OF` | `Incident` → `Case` | `as_of` | Distinct from `BELONGS_TO_CASE` (which every node gets directly) — this is the specific Incident-to-Case structural link the architecture calls out. |
| `OCCURRED_ON` | `Incident` → date (property, not a Date node — AGE has no native temporal type worth a node) | `locked` (bool), `locked_by`, `locked_at`, `as_of`, `superseded_by` | Timeline event. `locked=true` blocks any writer (including a resolution/extraction re-run) from superseding this edge until an investigator calls `unlock_event()` — see `src/graph/versioning.py`. |
