...
| 50 | G1/G6/CR3 — wiring Modules 31–36's aggregates into their plans | `MODULE50_RESULT.md` | done — six aggregates wired and firing on every live run; `_MAX_SUB_QUERIES` settled at **5** with the measured timeout data; G6 trades `_SQ_GENDER` for the arrest rate; defects split out as Modules 51–52 |
