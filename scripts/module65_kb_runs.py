"""
Module 65 — live KB1..KB9 runner (adapted from scripts/module52_kb_runs.py).
...
each attempt, every evaluator verdict, and the statute hypotheses generated.
...
Usage:
    GOLD32_BASE_URL=http://127.0.0.1:8024     M65_LOG=D:/Rapids AI/muhafiz-m65/backend.log     M65_OUT=.../runs.json M65_RUNS=3 M65_ARM=before     python scripts/module65_kb_runs.py
"""
...
LOG = os.environ.get("M65_LOG", os.path.join(HERE, "backend.log"))
OUT = os.environ.get("M65_OUT", os.path.join(HERE, "module65_runs.json"))
RUNS = int(os.environ.get("M65_RUNS", "3"))
ARM = os.environ.get("M65_ARM", "before")
IDS = [i for i in (os.environ.get("M65_IDS") or "").split(",") if i]
TIMEOUT_S = int(os.environ.get("GOLD32_TIMEOUT_S", "1200"))

KB_IDS = ["KB1", "KB2", "KB3", "KB4", "KB5", "KB6", "KB8", "KB9"]

# A NON-GOLD paraphrase of KB2 (section 6 of the result-file shape).
# Same facts, different words, and deliberately NOT gold's wording: it never
# says "data gap", never says "record ... what a witness said", and asks the
# admissibility question the other way round ("can it be used in court?").
# Measured to trip `_is_legal_kb_intent()` (True), so it reaches the same
# legal-KB path the fix sits on. Its gold is KB2's gold.
PARAPHRASE_ID = "KB2P"
PARAPHRASE = (
...
# A SECOND non-gold paraphrase, Roman-Urdu, to check the fix is not
# English-only. Same question, no gold vocabulary at all.
PARAPHRASE2_ID = "KB2Q"
PARAPHRASE2 = (
...
def main():
    ...
    gold[PARAPHRASE_ID] = {**gold["KB2"], "id": PARAPHRASE_ID, "question": PARAPHRASE}
    gold[PARAPHRASE2_ID] = {**gold["KB2"], "id": PARAPHRASE2_ID, "question": PARAPHRASE2}
    ids = IDS or KB_IDS
    out = []
