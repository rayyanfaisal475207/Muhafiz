# -*- coding: utf-8 -*-
"""
Module 143 — live `/api/chat` runner (adapted from scripts/module37_kb_runs.py).
...
window, whether the Module 143 retry gate fired ("zero-signal first pass"),
...
Usage:
    M143_BASE=http://127.0.0.1:8143 M143_LOG=backend.log \
    M143_OUT=scratchpad/m143_live.json M143_RUNS=3 M143_ARM=after \
    M143_IDS=M143,P1 PYTHONPATH=. python -X utf8 scripts/module143_live_runs.py

M143_IDS defaults to all 32 gold questions (the regression control). Ids not
...
BASE = os.environ.get("M143_BASE", "http://127.0.0.1:8143")
EMAIL = os.environ.get("EVAL_ADMIN_EMAIL", "admin@example.com")
PW = os.environ.get("EVAL_ADMIN_PASSWORD", "MuhafizAdmin2026!")
LOG = os.environ.get("M143_LOG", os.path.join(HERE, "backend.log"))
OUT = os.environ.get("M143_OUT", os.path.join(HERE, "scratchpad", "m143_live.json"))
RUNS = int(os.environ.get("M143_RUNS", "1"))
ARM = os.environ.get("M143_ARM", "before")
IDS = [i for i in (os.environ.get("M143_IDS") or "").split(",") if i]
TIMEOUT_S = int(os.environ.get("M143_TIMEOUT_S", "1500"))
    ...
    for run in range(1, RUNS + 1):
        for qid in ids:
            ...
            print(f"[{ARM} run{run}] {qid:8} route={str(p.get('last_dispatch_route')):9} "
                  ...
                  f"gen={p['generation'][0]:5} cutover={p['cutover_fallbacks']} "
