"""
Module 64 — live KB6 runner (adapted from scripts/module52_kb_runs.py).
...
Usage:
    ...
    M64_LOG=D:/Rapids AI/muhafiz-m64/backend.log \
    M64_OUT=.../runs.json M64_RUNS=3 M64_ARM=before \
...
LOG = os.environ.get("M64_LOG", os.path.join(HERE, "backend.log"))
OUT = os.environ.get("M64_OUT", os.path.join(HERE, "module64_runs.json"))
RUNS = int(os.environ.get("M64_RUNS", "3"))
ARM = os.environ.get("M64_ARM", "before")
IDS = [i for i in (os.environ.get("M64_IDS") or "").split(",") if i]
...
# The THREE non-gold paraphrases of §6. All three were written BEFORE they
# were run and were not adjusted afterwards. All three were checked against
# `_is_legal_kb_intent()` first — Module 52's own KB6P turned out to return
# False there, which voided it as a test — and all three return True.
#
# P3 is the generality control that matters most: a DIFFERENT exhibit type
# (a blood sample) in the SAME document. Rule 4d is about naming the exhibit
# type, not about firearms, and if it only worked for firearms it would be a
# gold-specific fix wearing a general rule's clothes.
PARAPHRASES = {
    "KB6-P1": (
        "Before a seized pistol is logged into our records, do the forensic "
        "guidelines lay down anything about the state it has to be in, and "
...
def main():
    ...
    for pid, ptext in PARAPHRASES.items():
        gold[pid] = {**gold["KB6"], "id": pid, "question": ptext}
    ids = IDS or KB_IDS
    out = []
