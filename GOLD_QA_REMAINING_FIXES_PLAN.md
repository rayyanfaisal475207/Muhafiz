...
| 158 | **A matched Meta-Analysis decomposition plan cannot select Meta-Analysis on its own** — `_xagg_answers_in_one_call()` lets the match veto the one-call skip, but `classify_to_subagent()` still needs a separate `_META_ANALYSIS_TRIGGER_PATTERNS` hit, and the two lists have been maintained in parallel since Module 29 | *(not yet branched — one clause in `supervisor.py`, Module 145's file)* | ⬜ New — found by Module 79, and it is why that module's live example is fixed in-process only. On the XAGG route a query whose text matches `most_cited_section_by_district` gets `one_call=False` and then falls through `elif allow_meta_analysis and any(_META_ANALYSIS_TRIGGER_PATTERNS)` to `else` → `_ROUTE_TO_SUBAGENT["XAGG"]` = Large-Scale Aggregate — measured live 1/1 (after-arm, same answer as before) and pinned deterministically in `module79_probe_dispatch.py`. The fix is one clause: a matched `_match_decomposition_plan()` IS a Meta-Analysis trigger (guarded by `allow_meta_analysis`, so the recursion guard is unchanged). CR3/G1/G6 are unaffected — each already has a trigger. Not made by Module 79 because `supervisor.py` is Module 145's this session; **the moment it lands, Module 79's two plans and their four pre-written paraphrases go live with no further change** |
...
| 164 | **The supervisor chokepoint (Modules 111/123's layer) is not built** — `_META_ANALYSIS_TRIGGER_PATTERNS` in `classify_to_subagent()` is untouched by Module 145 | *(not yet branched)* | ⬜ New — Module 145 §8. Same mechanism applies (describe the decomposition plans, score with the cross-encoder, fire above a measured threshold) with Module 116's six rewordings as the target set, but the plan definitions live in `meta_analysis.py` (owned by `muhafiz-m79` during Module 145) and the hazard set is the inverse of Module 145's — aggregate-shaped questions must NOT be decomposed. The G1-en rewording being Module 145's top hazard (0.351) is a preview of the difficulty |
...
| 178 | **PRIORITY — LLM-generated graph queries as a bounded, visible fallback for questions no aggregate answers and no plan composes** | `feat/llm-query-fallback` | ✅ **Done — built, five bounds each pinned by a test, shipped DISABLED (`LLM_QUERY_FALLBACK_ENABLED=false`) on the measurement.** `src/pipeline/llm_query_fallback.py` + one fallback branch in `supervisor.py` after the dispatch: fires only for XGRAPH/XNETWORK when keyword, semantic, sub-agent selection and plan matching all missed AND the route's default sub-agent came back EMPTY/ABSTAINED; one `call_llm()` (local Qwen3-14B first); allow-list grammar built from the live schema (labels, per-label properties, edge-direction census, enumerated values) rather than a keyword blacklist; case allow-list INJECTED as `BELONGS_TO_CASE → $case_ids`; row cap 200 / `statement_timeout` 15 s; runs on `muhafiz_mcp_readonly` (migration 033) inside a READ ONLY transaction that is **always rolled back**; served `PARTIAL` with the query verbatim in `caveats`, `tools_used=["LLM_QUERY"]` (*"generated graph query (unverified)"*) and a `citation_validator … unverified` event (the existing warning pill); prose still through `verify_grounding()`. **0 of 32 gold questions reach it** — structurally (test) and live (0 fallback events in 32 runs). **Measured (§4/§6):** Q2 0/3 served, Q3 0/3 served — the local model's Cypher fails AGE (`WITH` scoping) or the direction census, and its *intent* on Q2 returns 0 rows if repaired (a plausible "no" to a one-person "yes"); before the census guard existed, **2 of 2 served Q2 answers were wrong**. Of Module 92's 96 paraphrases 7 can reach the layer (all dispatch defects one layer up); it served 2 (D1::2 → 73, CR2::19 → the two multi-case accused), both right, both questions an aggregate already answers. Hazards: 0/10 reach it live; forced (`attempt()` called directly), 7/20 served of which 3 are wrong — H10 twice: a correct 67-row query narrated as *"not possible to determine"* and PASSED by the verifier (row 187) — and 2 more should have been refused (a 200-row officer roster to a password-hash request; a merge request answered as a read). **Wrong-but-plausible with the shipped guard: 3 of 9 served (33%) — 0/2 reached naturally, 3/7 on hazards; without the guard 2/2 on the target question — hence disabled.** **Row 183 (CRITICAL):** verifying bound 2 showed AGE 1.5.0 runs `SET`/`REMOVE`/`DETACH DELETE` under a SELECT-only role AND inside `BEGIN READ ONLY`; the first probe deleted every Case vertex and its edges from the live graph (restored exactly from the 2026-09-10 dump, ~11 min). Result: `docs/gold-qa-wave2-results/MODULE178_RESULT.md` |
