...
| 144 | **Count aggregates take no filters** — `fir_count` and siblings answer one fixed question each, so *"FIRs registered in 2019 or earlier"* returned the grand total, **73**, when the true answer is **0** | *(not yet branched)* | ⬜ New — filed 2026-09-14 from a live query. The FIRs span 2024–2026 (13 / 3 / 57), and the year constraint was silently dropped; the paraphrase verifier then rejected the summary and served the raw number, which was the right safety behaviour and also hid that the number was for the wrong question. **Fields verified in the graph:** `Incident.report_datetime` (date), `Person.age` (age), `fir_section` records (section, 218 across 36), `District` nodes (district). **City** has no separate field and folds into district; **province exists nowhere** in the graph and needs ingestion work first. The SQL route's param extractor already pulls dates and numbers from questions — reuse it. Multiplies every existing aggregate by every applicable filter |
