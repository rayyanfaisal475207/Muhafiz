...
| 151 | **The verifier rejects a correct answer that states an ABSENCE** — KB5 and KB9 were right and were discarded; KB2 is confidently wrong for the same reason | *(not yet branched)* | ⬜ New — filed 2026-09-14. On today's run KB9 (3/3 before) and KB5 (2/3 before) both came back `status=error` with the served text *"could not be verified as grounded"*. The verifier's own reasons: *"The claim about case records lacking data on inquest report compliance is unsupported"* and *"Claims about missing data fields in case records are not supported by any cited…"*. **Gold for both says the records do not hold that data. The system said so. The verifier rejected it because nothing in the document corpus can cite a negative.** Same mechanism is why KB2 asserts records *are* maintained: the true answer (a schema fact) is ungroundable, so the generator drifts to something citable. **Fix:** ground schema-shaped claims (*"no field for X"*, *"our records do not capture Y"*) against the live column/property inventory instead of the corpus — Module 95's `_missing_custody_controls()` is the existing precedent. Worth 2 questions directly and unblocks a third. **Do not** loosen the verifier: Module 82 measured it catching invented rule numbers, FIR numbers, compliance claims and a fabricated negative |
