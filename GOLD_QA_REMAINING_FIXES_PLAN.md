...
| 161 | **L2 (*citizen-service user also under investigation*) has no aggregate to reach** — it needs a person join across the PKM-applicant silo and the accused roster, which `graph_recurrence_person` (BELONGS_TO_CASE recurrence) is not | *(not yet branched)* | ⬜ New — Module 145 §4/§8. The cross-encoder's 0.002 for it is correct, not a miss; the capability is missing, not misrouted. Filed in row 145 as one of the two entity questions; it is a new-aggregate item, CR2's family but a different join |
