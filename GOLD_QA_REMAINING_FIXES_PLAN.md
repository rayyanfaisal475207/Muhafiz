...
# Module 70 — the paraphrase gate had no opposite direction

Branch `fix/paraphrase-omission-check`, from `origin/main` @ `21d3d0e`.
Full write-up: `docs/gold-qa-wave2-results/MODULE70_RESULT.md`.

## What was wrong

`verify_structured_aggregate_paraphrase()` computed exactly one numeric test,
`ans_nums - src_nums`. That set difference is one-directional by construction:
it catches every number a paraphrase *invents* and is structurally incapable of
noticing one it *omits*. Nothing anywhere checked that the figures the
aggregate computed survived into the served answer. Module 83 measured the cost
on M2 — headline computed and rendered on 9 of 9, dropped from the served answer
on 9 of 9 — and left the fix open because the gate is shared by most
XAGG-composing questions.

## The class question, answered

Module 83 only looked at M2. All 32 gold questions were run through the gate
before anything was designed. **25 compose it, across 19 aggregate kinds, and
exactly 1 — M2 — drops a load-bearing figure.** The naive rule the tracker row
implies fires on **10 of 32** and is wrong on at least 5: it reads statute years
out of a `NOTE:` caveat line as computed figures, and it cannot see that an Urdu
answer stated its figure as a word. This is an M2 fix that generalises, not a
class fix, and saying otherwise would have overstated it.

## What the rule is

Only an `X of Y` proportion in the **first rendered line**. Two properties
already load-bearing elsewhere in the codebase make that identifiable without
hand-listing anything per question: every renderer leads with its finding
(`_render_aggregate_text()`'s own comment), and a renderer writes "X of Y" only
for a computed proportion — the one numeric construction destroyed by dropping
either half. A count survives being summarised away; "2 stations" without "of
19" is a different and duller claim. Nothing below the headline is covered,
because Module 104 records that a list ordinal is itself read as a claimed
figure, so a rule that pushes answers into list shape trips a different gate.

## What happens when it fires — not a rejection

Module 83 proposed serving `raw_summary_text`. **That was not built.** A rule
that cannot be perfect must not be able to turn a good prose answer into a
computed dump — that is what `[PRESERVE]` did to KB9 and what Module 101 had to
undo. Instead: the verifier signal is purely additive (`grounded` and
`unsupported_claims` cannot move — proven over 900 cells against
`origin/main`), and the sub-agent does **one** repair regeneration with the
missing figures computed at runtime from the rendering, adopting it only if it
passes the full gate — invented-number direction included — and omits strictly
fewer. The branch is structurally incapable of making a served answer worse.

## Measured

M2 **0 of 9 → 9 of 9**, all generations local `qwen3:14b`; judge
FactualCorrectness **0.20 → 1.00 on 3 of 3**. Non-gold Urdu 3/3 and Roman-Urdu
3/3. Answers grew 301 → 399 chars with **zero** new list lines. All-32: routes
32/32 identical, repair fires on 1 of 32, and the two questions that moved were
resampled n=3 per arm and shown to be the gate's own pre-existing
invented-number flap (KB1 is *worse* on `origin/main`: 3/3 rejected vs 1/3).

## What this does NOT buy

A natural English rewording of M2 never reaches M2's aggregate at all — 3 of 3
runs resolve to a flat per-station listing (Defect 122). The finding is only
reachable under wordings close to gold's, and the fix repairs the paraphrase of
a finding that was computed. New defects **122–125**.
