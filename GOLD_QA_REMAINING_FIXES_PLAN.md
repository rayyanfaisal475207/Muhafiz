...
| 150 | **Meta-Analysis's per-sub-query deadline sends concurrent sub-queries to Groq on the 14B model** — 6 of 134 LLM calls in today's 32-question run, all inside two plans | *(not yet branched)* | ⬜ New — filed 2026-09-14 from the post-tunnel-fix run. G1's `caseload_review` lost 1, G6's `orientation_note` lost **5 within two seconds** — five concurrent sub-queries at ~45 s per local call cannot all finish inside the deadline (`config.py:143`, Module 10), so the slow ones are cancelled and fall to Groq with an **empty** error message (the signature of a cancelled task, not a model failure). Module 110 raised the plan cap 5→8, which widened the fan-out. **Cheap:** the fix is a concurrency limit or a deadline that scales with fan-out, not a code path. Every other question ran 100% local |
