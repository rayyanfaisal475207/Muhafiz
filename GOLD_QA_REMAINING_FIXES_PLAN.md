...
| 143 | **An unanswerable RAG question costs ~108 s to abstain** — the retry loop refines and re-retrieves twice against a corpus that scored zero relevance on the first pass | *(not yet branched)* | ⬜ New — filed 2026-09-14 from a live query (*"How often is the officer who registers an FIR also the officer who investigates the case?"*), which fell through to Semantic Search and took **108.4 s** to say *"No sufficiently relevant documents were found"*. Three full retrieve-evaluate-generate cycles for a question whose answer was never in a document. Retries make sense for a near-miss; a **zero-signal** first pass should abstain in seconds. Changes no answer, only the cost of every unanswerable question |
