...
**Status:** **Implemented and live.** All eleven sub-agents are built and running; this
deployment's `.env` sets `HARNESS_CUTOVER_ROUTES` to all seven retrieval routes, and every
figure in `evaluation/GOLD32_EVALUATION_REPORT_2026-09-10.md` was produced through the
harness. This document is retained as the **design record and preservation contract** — it
is cited by ~25 source files as the rationale for specific behaviour, so the requirements
below remain binding on any future change.
