...
class DataGateway(Protocol):
    ...
    async def get_sessions_for_user(self, user_id: str, project_id: str = None, case_id: str = None) -> list[dict]: ...
    ...
    async def log_step(self, run_id: str, step_name: str, step_order: int, status: str, duration_ms: int = None, input_summary: dict = None, output_summary: dict = None) -> None: ...
    ...
    async def get_cases(self, user_id: str, user_role: str) -> list[dict]: ...
    # [Gold-QA fix follow-up] Resolves a case's human-facing display number
    # ("435/26" — Case.fir_number) to its internal case_id ("fir-435-26" —
    # Case.case_id, a DIFFERENT string, no fixed transform between them).
    # Existence-only, no RBAC filtering — callers that need access control
    # still call check_case_access() with the resolved case_id afterward
    # (see src/pipeline/harness/supervisor.py's own case-scope-inference
    # use of this).
    ...
    # [Reconciliation fix — harness-reconciliation Unit 6] Deliberately a
    # separate method from update_case(), not a whitelisted field on it:
    # update_case()'s `allowed` set is the case-EDIT surface (fir_number,
    # investigation_status, etc.) reachable from the case-edit API. This
    # marker is internal, written only by the conflict-detection background
    # task — adding it to update_case()'s whitelist would make it settable
    # through that same edit endpoint, which is not an access path this
    # marker should have.
    ...
    async def get_errors_since(self, since: str, max_rows: int = 50_000) -> list[dict]: ...
    ...
    async def get_runs_since(self, since: str, max_rows: int = 50_000) -> list[dict]: ...
    async def get_step_latencies_since(self, since: str, max_rows: int = 50_000) -> list[dict]: ...
